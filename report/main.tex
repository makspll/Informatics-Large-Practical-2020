...
\begin{mitem}
    \scriptsize
    \mitemxx{\textbf{AQSensor}(SensorData sensorData, W3WAddressData w3WAddressData)}
...
\begin{mitem}
    \scriptsize
    \mitemxx{\textbf{AQWebServerClient}(HttpClient client, URI APIBaseURI)}
    {Construct a new web server client served via the given http client and the api uri}
    \mitemxx{mapbox::FeatureCollection \textbf{fetchBuildings}}
    {Retrieves the feature collection of all the no-fly zones available from the server}
    \mitemxx{List$<$SensorData$>$ \textbf{fetchSensorsForDate}(LocalDate date)}
    {Retrieves the data of all sensors to be collected on the given date}
    \mitemxx{W3WAddressData \textbf{fetchW3WAddress}(String wordAddress)}
...
\begin{mitem}
    \scriptsize
    \mitemxx{\textbf{SensorData}(String W3WLocation, float battery, float reading)}
    {Constructs a new sensor data object}
    \mitemxx{getters for all attributes}
    {-}
\end{mitem}
...
\begin{mitem}
    \scriptsize
    \mitemxx{\textbf{W3WAddressData}(String country, W3WSquareData square, String nearestPlace, mapbox::Point coordinates, String words, String language, String map)}
...
\begin{mitem}
    \scriptsize
    \mitemxx{\textbf{W3WSquareData}(mapbox::Point southwest, mapbox::Point northeast)}
