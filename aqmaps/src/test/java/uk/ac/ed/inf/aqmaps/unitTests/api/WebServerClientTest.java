package uk.ac.ed.inf.aqmaps.unitTests.api;
...
public class WebServerClientTest {
    ...
    DroneWebServerClient testClient = new DroneWebServerClient(URI.create("http://localhost:9898"));
    ...
    public void testFetchSensorsForDateCorrectData() throws IOException, InterruptedException {
        ...
        Map<String,SensorData> correctOutputs = Map.ofEntries(
            entry("hooked.shine.third",new SensorData("hooked.shine.third",5.542588f,Float.NaN)),
            entry("sports.topic.clocks",new SensorData("sports.topic.clocks",42.300785f,194.9f)),
            entry("noted.friday.jams",new SensorData("noted.friday.jams",9.161681f,Float.NaN))

        );

        List<SensorData> sensors = testClient.fetchSensorsForDate( LocalDate.of(2020,1,1));

        for (SensorData sensor : sensors) {
            if(correctOutputs.containsKey(sensor.getLocation())){
                SensorData correctOutput = correctOutputs.get(sensor.getLocation());
    ...
    public void testFetchW3WAddress() throws IOException, InterruptedException {
        ...
        Map<String,W3WAddressData> correctOutputs = Map.ofEntries(
            entry("rated.fired.crowds", new W3WAddressData(  "GB", new W3WSquareData(
        ...
        for (String address  : correctOutputs.keySet()) {
            ...
            W3WAddressData output = testClient.fetchW3WAddress(words[0], words[1], words[2]);
